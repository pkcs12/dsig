...
func NewSigner(xml string, external *crypto.Signer) (*Signer, error) {
	...
	s := &Signer{
		signatureData: signatureData{xml: doc},
		external:      external,
	}
	return s, nil
}
...
// HashFunc renders conformance to crypto.SignerOpts
func (s *Signer) HashFunc() crypto.Hash {
	return signingAlgorithms[s.sigAlgorithm].hash
}
...
func (s *Signer) setSignature() error {
	...
	signature, err = (*s.external).Sign(rand.Reader, hashed, s)
